...
## GraphQL Integration with AshGraphql

This example shows how to integrate translations with AshGraphql using Absinthe schema extensions.
...
defmodule MyApp.Shop.Product do
  ...
  translations do
    ...
    backend :database
    graphql_translations true  # Mark for GraphQL extensions
  end
...
```bash
mix ash_phoenix_translations.gen.graphql_extensions --domains MyApp.Shop
```

This generates `lib/my_app_web/schema/translation_extensions.ex`:
...
defmodule MyAppWeb.Schema.TranslationExtensions do
  ...
  # Shared types used by all translation fields
  ...
  # Extensions for MyApp.Shop.Product
  extend object(:product) do
    ...
    field :name_translation, :string do
      ...
      resolve fn resource, args, resolution ->
        AshPhoenixTranslations.Graphql.resolve_translation(
          resource,
          args,
          Map.put(resolution, :state, :name)
        )
      end
    end

    # All translations for name field
    ...
    # Singular translation field for description
    field :description_translation, :string do
      ...
      resolve fn resource, args, resolution ->
        AshPhoenixTranslations.Graphql.resolve_translation(
          resource,
          args,
          Map.put(resolution, :state, :description)
        )
      end
    end

    # All translations for description field
...
### Step 3: Configure Your Absinthe Schema

```elixir
defmodule MyAppWeb.Schema do
  use Absinthe.Schema
  use AshGraphql, domains: [MyApp.Shop]

  # Import the generated translation extensions
  import_type_extensions MyAppWeb.Schema.TranslationExtensions

  query do
    # Queries are automatically defined by AshGraphql based on your resource configuration
  end
end
```

### Step 4: GraphQL Query Examples

```graphql
# Query products with Spanish translations
query {
  listProducts {
    id
    sku
    ...
    descriptionTranslation(locale: "es")
  }
}
...
# Mix different locales in same query
query {
  listProducts {
    id
...
query {
  getProduct(id: "123") {
    id
    nameTranslation(locale: "de")  # Falls back to English if German not available
  }
}

# Access all translations to build a translation UI
...
### Advanced: Custom Locale Context Middleware

Add middleware to automatically set locale from request headers:
...
defmodule MyAppWeb.Schema do
  ...
  def context(ctx) do
    # Extract locale from request headers or session
    locale = get_in(ctx, [:headers, "accept-language"]) || :en
    Map.put(ctx, :locale, locale)
  end

  query do
    # Queries defined by AshGraphql
  end
end
```
    ...
    "listProducts": [
      {
        ...
        "sku": "PROD-001",
        "price": "29.99",
        "nameTranslation": "Producto Ejemplo",
        "descriptionTranslation": "Una descripción en español"
      },
      {
        "id": "456",
        "sku": "PROD-002",
        "price": "39.99",
        "nameTranslation": "Otro Producto",
        "descriptionTranslation": "Otra descripción"
      }
    ]
  }
}
